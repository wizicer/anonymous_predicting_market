...
\begin{itemize}
    \item $ct_i \leftarrow Enc(pk, side_i \| address_i)$: 阈值加密的下注方向的密文，其中 $side_i$ 是下注的方向，$address_i$ 为用户地址，为 $160$ 位。
    \begin{itemize}
        \item 这里我们采用 ElGamal 加密方案。
        \item 将消息 $side_i \| address_i$ 映射到椭圆曲线上，\[m \leftarrow g^{side_i \times 2^{160} + address_i \mod q}\]
        \item 计算 \[\beta \overset{R}{\leftarrow} \mathbb{Z}_q, \quad v \leftarrow g^{\beta}, \quad e \leftarrow m + pk^{\beta}\]
        \item 输出 \[ct_i = (v, e) \in \mathbb{G} \times \mathbb{G}\]
    \end{itemize}
...
\begin{itemize}
    ...
    \item $\left(\pi_{batch}, sum_0, sum_1 \right) \leftarrow BatchProve\left(pk, salt, \{(comm_i, ammount_i)\}_{i = 1}^M;\{side_i,address_i\}_{i = 1}^M\right)$:生成批量证明并输出聚合金额，在电路中：
    \begin{enumerate}
